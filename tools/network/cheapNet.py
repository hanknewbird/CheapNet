...
def encoder_block(in_channels, out_channels, groups, slope):
    block = nn.Sequential(
...
def decoder_block(in_channels, out_channels, groups, slope):
    block = nn.Sequential(
...
class CheapNet(nn.Module):
    def __init__(self, groups, slope=0.2):
        super(CheapNet, self).__init__()

        # [b,1,512,512] ==> [b,groups,512,512]
        self.encoder1 = nn.Sequential(
            nn.Conv2d(1, groups, (3, 3), stride=(1, 1), padding=(1, 1), groups=1),
            nn.BatchNorm2d(groups),
            nn.LeakyReLU(slope)
        )

        # [b,groups,512,512] ==> [b,groups,256,256]
        self.encoder2 = encoder_block(groups, groups, groups, slope)

        # [b,groups,256,256] ==> [b,groups,128,128]
        self.encoder3 = encoder_block(groups, groups, groups, slope)

        # [b,groups,128,128] ==> [b,groups,64,64]
        self.encoder4 = encoder_block(groups, groups, groups, slope)

        # [b,groups,64,64] ==> [b,groups,32,32]
        self.bottleneck = encoder_block(groups, groups, groups, slope)

        # [b,groups,32,32] ==> [b,groups,64,64]
        self.decoder1 = decoder_block(groups, groups, groups, slope)

        # [b,groups,64,64] ==> [b,groups,128,128]
        self.decoder2 = decoder_block(groups*2, groups, groups, slope)

        # [b,groups,128,128] ==> [b,groups,256,256]
        self.decoder3 = decoder_block(groups*2, groups, groups, slope)

        # [b,groups,256,256] ==> [b,groups,512,512]
        self.decoder4 = decoder_block(groups*2, groups, groups, slope)
